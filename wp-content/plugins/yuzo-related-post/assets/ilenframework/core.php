...
if ( !class_exists('ilen_framework_2_9') ) {

class ilen_framework_2_9 {
		...
		echo '
			<script>
				jQuery.ajax({
			        dataType: "json",
			        type: "POST",
			        url: "http://yuzopro.com/active.php?veri='.$temp_options->active_license.'&callback=?",
			        success: function (html) {
			            for ( var i in html ) {
			                if( html.validate == 0 ){
								...
								createCookie("license_validate_daily_yz",1,10);
			                    console.log("okok");
			                }
			            }
